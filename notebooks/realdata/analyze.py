...
ax1.set_title("Standardized bootstrap deviations vs. $\\mathcal{N}(0,1)$")
...
ax2.set_title("Bootstrap SE recovers analytic SE")
...
ax3.set_title("Detection robustness of low-abundance calls")
ax3.legend(frameon=False, fontsize=8, loc="center right")

n_persist = int(persist.sum())
print(f"persist (pi>=0.95)                : {n_persist}")

fig.tight_layout()
# Panel letters at the top-left corner of each panel, outside the axes.
for ax, letter in zip((ax1, ax2, ax3), "ABC"):
    bbox = ax.get_tightbbox(fig.canvas.get_renderer()).transformed(fig.transFigure.inverted())
    fig.text(bbox.x0, bbox.y1, letter, fontsize=14, fontweight="normal", ha="left", va="top")
